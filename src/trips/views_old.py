...
class TripMemberListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def test_func(self):
        ...
        return self.trip.is_owner(self.request.user) or self.trip.is_participant(self.request.user)

    def handle_no_permission(self):
        messages.error(self.request, 'You cannot view members of this Trip - only members and owners have access.')
        return redirect('trip-list')
